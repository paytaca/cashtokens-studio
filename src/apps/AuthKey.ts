...
export class AuthKey implements UtxoI {
  ...
  static get transferCost(): number {
    return (
      calcMinerFee({ P2PKH: 2 }, { P2WSH: 1, P2PKH: 1 }) + DEFAULT_TOKEN_VALUE
  ...
  protected prepareChangeReq(funderUtxo: UtxoI): SendRequest[] {
    ...
    if (this.satoshis - this.genesisCost > 100) {
      // if there is non-negligible change
      // Change
      return [
        new SendRequest({
  ...
  protected async buildTokenGenesisTransaction(
    genesisRequests: (TokenSendRequest | SendRequest | OpReturnData)[],
    ...
    if (!funderUtxo) {
      if (this.satoshis <= this.genesisCost) {
        ...
        throw new Error(
          'Insufficient balance to fund the transaction. Please try to consolidate your utxos.',
        );
      }
  ...
  async createGenesis(opt?: {
    ...
    try {
      const { encodedTransaction, sourceOutputs } =
        await this.buildTokenGenesisTransaction(requests);
      this._processing = 'Waiting for signature';
   ...
   */
  static async scanWalletForAuthKeys(
    ownerWallet: Wallet,
  ): Promise<AuthKey[] | undefined> {
    AuthKey._processing = 'Scanning wallets for AuthKeys';
    const authKeyUtxos = (await ownerWallet?.getAddressUtxos()).filter(
      (u: UtxoI) => u.token && u.token.commitment == AuthKey.DEFAULT_COMMITMENT,
    );
    const authKeys = [];
  ...
  async transfer(newOwner: string): Promise<string | undefined> {
    ...
    const funderUtxo = (await this.ownerWallet!.getAddressUtxos()).filter(
      (u: UtxoI) => {
        return Boolean(!u.token) && u.satoshis > this.transferCost;
      },
    )[0];
