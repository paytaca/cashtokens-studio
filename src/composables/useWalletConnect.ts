...
export const useWalletConnect = () => {
  ...
  const walletConnectConnect = async () => {
    try {
      const connectedChain =
        user.walletNetworkType == 'mainnet' ? 'bch:bitcoincash' : 'bch:bchtest';
      ...
      if (walletConnectSession.value) {
        if (walletConnectSession.value?.namespaces?.bch?.accounts) {
          address =
            walletConnectSession.value.namespaces.bch.accounts[0].replace(
              'bch:',
              '',
            );
          address = formatAddress(address);
  ...
  const walletConnectDisconnect = async () => {
    if (walletConnectSession.value?.topic) {
      try {
        await walletConnectSignerClient.value?.disconnect({
  ...
  const walletConnectSignMessage = async (
    message: any,
    broadcast?: boolean,
    prompt?: string,
  ): Promise<any> => {
