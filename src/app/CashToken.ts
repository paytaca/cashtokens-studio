...
export class CashToken implements UtxoI, PartialBcmr {
  ...
  protected prepareGenesisAuthchainIdentityReq(args: {
    recipient: string;
    token: TokenI;
  }): TokenSendRequest {
    const reqParam = {
  ...
  protected prepareGenesisRegistryPublicationReq(): OpReturnData[] {
    if (this.registry) {
      // if registry is set, assumes publishing
  ...
  protected async buildTokenGenesisTransaction(
    ...
    const { encodedTransaction, sourceOutputs } =
      await this.ownerWallet!.encodeTransaction(genesisRequests, false, {
  ...
  async createGenesis(opt: GenesisOptions): Promise<string | undefined> {
    ...
    requests.push(
      this.prepareGenesisAuthchainIdentityReq({
        ...
        token: {
          ...
          capability:
            opt.amount && BigInt(opt.amount) > 0 && !opt.capability
  ...
  async buildAuthChainInChainGraph(): Promise<AuthChain> {
    ...
    const authChain = await BCMR.buildAuthChain({
      transactionHash: this.token?.tokenId || this.utxo.txid,
      network: this.ownerWallet!.network,
      followToHead: true,
    });
    delete this._processing;
    return authChain;
  ...
  static async send(arg: {
    tokenId: string;
    amount: bigint;
    to: string;
    capabality?: NFTCapability;
    commitment?: string;
    ownerWallet: Wallet;
    ...
    const { encodedTransaction, sourceOutputs } =
      await arg.ownerWallet!.encodeTransaction(requests, false, {
        tokenOperation: 'send',
    ...
    try {
      this._processing = 'Waiting for signature';
      signResult = await arg.transactionSigner?.signTransaction(
  ...
  static async scanWalletForTokens(
    ...
    return (
      (await ownerWallet.getAddressUtxos()).filter((u: UtxoI) => u.token) || []
    );
  }
  ...
  async transferNFT(arg: { newOwner: string }) {
    ...
    try {
      const { encodedTransaction, sourceOutputs } =
        await this.ownerWallet!.encodeTransaction(requests, false, {
          tokenOperation: 'send',
          checkTokenQuantities: false,
          buildUnsigned: true,
        });
