...
export class Watchtower {
   ...
   */
  async fetchAuthchainIdentities(
    address: string,
    q?: FetchAuthchainIdentitiesQueryParams
  ...
  async fetchAuthhead(authbase: string): Promise<FetchAuthheadResponse> {
    let b;
    try {
      const r = await fetch(
        `${this.apiBaseUri}cts/authhead/?authbase=${authbase}`
      );
      b = await r.json();
    } catch (error) {
      this.error = error;
