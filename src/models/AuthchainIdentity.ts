...
export default class AuthchainIdentity implements CashStudioTokenI, AuthChainGuard {
  ...
  get contractScriptModified(): string {
    return `
    // AuthchainGuard v1.0.1
    ...
    contract AuthchainGuard(
      ...
      function PublishOrBurnOrReleaseOrTransfer(
        ...
        if (moveToSelf) {
          ...
          if (tx.outputs.length > 1 && tx.outputs[1].tokenCategory == tx.outputs[0].tokenCategory){
            // Ft Issue
            require(tx.outputs[1].tokenAmount <= tx.outputs[0].tokenAmount);
          }
          // Publish
  ...
  async release(recipient: string): Promise<string|undefined> {
    this.ensureTokenId()
    this.ensureContract()
    this.ensureOwnerWallet()
    this._processing = 'Processing'
  ...
  async issueFungibleTokens(amount:string, to:string) {
    ...
    if (!identityOutput.token?.amount || identityOutput.token.amount < BigInt(amount)) {
      throw new Error('Fungible reserve does not have enough supply')
    }

    const funderInput
      = (await this.ownerWallet!.getAddressUtxos()).filter((utxo: UtxoI) => Boolean(!utxo.token) && utxo.satoshis > 5000).map(toCashScript)[0]

    if (!funderInput) {
      throw new Error('insufficient balance')
    }
    this._processing = 'Processing transfer'
    const byteCount = getByteCount({'P2SH-P2WPKH':1, P2PKH:1}, {P2WSH:1, P2PKH:2}) // generous
    const minerFee = Math.ceil(byteCount * 1.1) + 500 + 1000
    let transaction
    let decoded
    const sig = new SignatureTemplate(Uint8Array.from(Array(32)))

    // const newOwnerAuthchainGuardContract = new Contract(this.contractScript, [this.ownerWallet!.getPublicKeyHash(false)], this.ownerWallet!.network)
    // const newOwnerWallet = await (getWalletClass()).watchOnly(newOwnerAuthchainGuardContract.getTokenDepositAddress())
    try {
      // Use publish path
      transaction =
        this.publishOrBurnOrReleaseOrTransfer(Uint8Array.from(Array(33)), Uint8Array.from(Array(65)), '0x')
          ...
          .to([{
            ...
            amount: identityOutput.satoshis,
            token: {
              category: identityOutput.token.category,
              amount: identityOutput.token!.amount - BigInt(amount),
              nft: identityOutput.token?.nft
            }
          }])
          .to([{
          ...
          .to([{
            to: this.ownerWallet!.getDepositAddress(),
            amount: funderInput.satoshis - BigInt(minerFee) - BigInt(1000) // token dust value
          }])
          .withoutChange().withoutTokenChange().withHardcodedFee(BigInt(minerFee))
    ...
    try {
      ...
      signed = await window.paytaca!.signTransaction({
        ...
        broadcast: false,
        userPrompt: 'Issue fungible tokens'
      });
    ...
    // Tx signing success, submitting transaction
    this._processing = 'Issuing'
    try {
      const tx = await this.ownerWallet!.submitTransaction(hexToBin(signed!.signedTransaction), true);
      this._message = { type: 'success', text: 'Success!Token issued. Tx=' + tx}
      return tx
    } catch (error) {
      console.log('Error issuing FT Token during submission of txn', error)
    } finally {
      delete this._processing
    }
  }
