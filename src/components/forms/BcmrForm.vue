<template>
  <q-form disable>
    <div
      v-if="bcmr"
      class="row q-mb-md rounded-borders"
    ...
    >
      <div class="col-12 q-gutter-sm q-py-sm row items-center justify-end">
        <q-btn
          ...
          flat
          :color="readOnly ? 'negative' : 'secondary'"
        >
          <template v-slot:default>
            <span v-if="!readOnly"
              ...
              <q-tooltip
                ...
                self="top middle"
                :offset="[10, 10]"
                >Click to disable edit</q-tooltip
              >
            </span>
            <span v-else
        ...
        >
          <template v-slot:default>
            <span v-if="$q.screen.gt.xs" class="q-ml-xs"
              >Download Registry</span
            >
          </template>
        </q-btn>
        ...
        <q-btn
          v-if="bcmr?.isModified"
          color="primary"
          size="md"
          @click="storeRegistryInIpfs"
          dense
          no-caps
          :icon="!saved ? 'save_as' : 'done_all'"
          :loading="Boolean(bcmr?.processing)"
        ...
        >
          ...
          <template v-slot:loading>
            <div class="row flex"><q-spinner-box></q-spinner-box>Saving</div>
          </template>
        </q-btn>
        <q-btn
          v-if="bcmr?.isModified && saved"
          color="primary"
          size="md"
          @click="
            () =>
              openBcmrPublisherDialog(
                AuthchainRegistryPublisherDialog.__name,
                bcmr?.authchainIdentity,
              )
          "
          dense
          no-caps
          :icon="updatePublished ? 'done_all' : 'publish'"
          :disable="Boolean(bcmr?.processing)"
          flat
        >
          <template v-slot:default>
    ...
    <q-banner
      ...
      :class="!$q.dark.isActive ? 'bg-grey-3' : 'bg-grey-10'"
      rounded
    >
      <q-expansion-item
        label="Registry"
        class="q-px-md q-pt-sm q-my-sm"
        icon="menu_book"
      >
        <div class="q-mx-md q-gutter-sm q-my-md">
          <q-input
            class="registry-field"
            @update:model-value="(v: any) => bcmr?.setSchema(v)"
            :model-value="bcmr?.$schema"
            label="Schema"
            filled
            dense
            disable
          ...
          <q-input
            class="registry-field"
            @update:model-value="(v: any) => bcmr?.setVersion(v)"
            :model-value="bcmr?.versionString"
          ...
          <q-input
            class="registry-field"
            :model-value="bcmr?.latestRevision"
          ...
          <q-input
            class="registry-field"
            @update:model-value="(v: any) => bcmr?.setLicense(v)"
            :model-value="bcmr?.license"
            label="License"
            placeholder="Example: CC0-1.0"
            aria-placeholder="Example: CC0-1.0"
            filled
            dense
          ></q-input>
        </div>
      ...
      >
        <div class="q-mx-md q-gutter-sm q-my-md">
          <q-input
            class="registry-field"
            @update:model-value="(v: any) => bcmr?.setTokenIdentityName(v)"
            :model-value="bcmr?.identitySnapshot?.name"
            label="Token identity name"
            filled
            dense
          ></q-input>
          <q-input
            class="registry-field"
            @update:model-value="
              (v: any) => bcmr?.setTokenIdentityDescription(v)
            "
            :model-value="bcmr?.identitySnapshot?.description"
            label="Token identity description"
            filled
            dense
          ></q-input>
        </div>
      ...
      <q-expansion-item
        label="Token Identity URIs (Links)"
        class="q-px-md q-pt-sm q-my-sm"
        icon="public"
      >
        <div class="q-mx-md q-gutter-sm q-my-md">
          <div
            v-for="(uriName, i) in Object.keys(
              bcmr?.identitySnapshot?.uris || {},
            )"
            :key="i"
          >
            <q-input
              input-class="registry-field"
              @update:model-value="(v: any) => bcmr?.setUri(uriName, v)"
              :model-value="bcmr?.identitySnapshot?.uris?.[uriName]"
              :label="uriName"
              filled
              dense
            />
        ...
        <div class="text-right">
          <q-btn
            @click="openAddLinkDialog(AddBcmrLinkDialog.__name, {})"
            :label="!bcmr?.identitySnapshot?.uris ? 'Add Links' : 'Edit Links'"
            color="secondary"
            dense
            flat
        ...
        <AddBcmrLinkDialog
          v-if="Boolean(bcmrLinkAdderDialog)"
          :model-value="bcmrLinkAdderDialog == AddBcmrLinkDialog.__name"
          @close="hideBcmrLinkAdderDialog"
          :links="bcmr?.identitySnapshot?.uris"
          @confirm="onConfirmAddLink"
          persistent
        />
      </q-expansion-item>
      <q-expansion-item
        label="Token Category Details"
        class="q-px-md q-pt-sm q-my-sm"
        icon="token"
      >
        <div class="q-mx-md q-gutter-sm q-my-md">
          <q-input
            class="registry-field"
            @update:model-value="(v: any) => bcmr?.setTokenSymbol(v)"
            :model-value="bcmr?.identitySnapshot?.token?.symbol"
            label="Token category symbol"
            filled
            dense
          ></q-input>
          <q-input
            class="registry-field"
            v-if="Number(bcmr?.authchainIdentity?.token?.amount) > 0"
            @update:model-value="(v: any) => bcmr?.setTokenDecimals(v)"
            :model-value="bcmr?.identitySnapshot?.token?.decimals"
            label="Token category decimals"
            filled
            dense
          ></q-input>
        </div>
      ...
      <q-expansion-item
        v-if="bcmr?.nfts && bcmr?.nfts.length > 0"
        label="Token Category NFTs"
        class="q-px-md q-pt-sm q-my-sm scroll overflow-auto"
        icon="token"
        style="max-width: 100%"
      >
        <div class="col-xs-12 scroll overflow-auto">
          <div class="q-pa-lg flex flex-center">
            <q-pagination
              v-model="nftsPagination.currentPage"
              :max="nftsPagination.numberOfPages"
              :max-pages="nftsPagination.maxRowsPerPage"
              :boundary-numbers="false"
            />
          </div>
          <q-markup-table
            style="max-width: 100%"
            class="overflow-auto text-wrap text-center"
          >
            <thead>
              <th>#</th>
              <th>Image</th>
              ...
              <tr v-for="(nft, i) in nftPage" :key="Object.keys(nft)[0] + i">
                <td>
                  {{
                    i +
                    1 +
                    nftsPagination.maxRowsPerPage *
                      (nftsPagination.currentPage - 1)
                  }}
                </td>
                <td>
    ...
    <AuthchainRegistryPublisherDialog
      v-if="dialog"
      :model-value="dialog === AuthchainRegistryPublisherDialog.__name"
      :authchain-identity="dialogData as AuthchainIdentity"
      :url="savedArtifact?.artifact?.uris?.https"
      :content-hash="savedArtifact?.contentHash"
      @hide="onHide"
      @registry-published="onRegistryPublished"
    />
  </q-form>
</template>
...
const {
  dialog,
  dialogData,
  openDialog: openBcmrPublisherDialog,
  onHide,
  hideDialog: hideBcmrPublisherDialog,
} = useDialogs();
const props = defineProps<{ registry?: Bcmr; readOnly?: boolean }>();
const bcmr = ref<Bcmr>();
const registryStorageArtifacts = ref<
  { contentHash: string; artifact: any }[] | null
>();
const {
  dialog: bcmrLinkAdderDialog,
  openDialog: openAddLinkDialog,
  hideDialog: hideBcmrLinkAdderDialog,
} = useDialogs();
...
const saved = computed(() => {
  if (bcmr.value && registryStorageArtifacts.value) {
    return Boolean(
      registryStorageArtifacts.value?.find(
        (a) => a.contentHash === bcmr.value!.getContentHash(),
      ),
    );
  }
  return false;
...
const savedArtifact = computed(() => {
  if (bcmr.value && registryStorageArtifacts.value) {
    return registryStorageArtifacts.value?.find(
      (a) => a.contentHash === bcmr.value!.getContentHash(),
    );
  }
  return null;
...
const initNftsPagination = () => {
  if (bcmr.value?.nfts && bcmr.value.nfts.length > 0) {
    if (bcmr.value.nfts.length > 10) {
      nftsPagination.value.numberOfPages = Math.ceil(
        bcmr.value.nfts.length / nftsPagination.value.maxRowsPerPage,
      );
    } else {
...
const updateNftPage = () => {
  if (nftsPagination.value.numberOfPages <= 1) {
    ...
    nftPage.value = bcmr.value!.nfts!.slice(
      ...
      nftsPagination.value.currentPage * nftsPagination.value.maxRowsPerPage +
        nftsPagination.value.maxRowsPerPage,
...
watch(
  () => nftsPagination.value.currentPage,
  (v) => {
    updateNftPage();
  },
);
...
onBeforeUnmount(() => {
  if (
    registryStorageArtifacts.value &&
    registryStorageArtifacts.value.length > 0
  ) {
    localStorage.setItem(
      'registryStorageArtifacts',
      JSON.stringify(registryStorageArtifacts.value),
    );
  }
});
...
const storeRegistryInIpfs = async () => {
  try {
    ...
    if (bcmr.value?.authchainIdentity?.authKey?.token?.tokenId) {
      bcmr.value!.appendAuthGuardTokenStandardExtension(
        bcmr.value?.authchainIdentity?.authKey?.token?.tokenId,
      );
    }
    const artifact = await bcmr.value?.storeRegistry();
    if (artifact) {
      registryStorageArtifacts.value?.push({
        contentHash: bcmr.value!.getContentHash(),
        artifact,
      });
      localStorage.setItem(
        'registryStorageArtifacts',
...
const downloadBcmr = async () => {
  if (bcmr.value) {
    const blob = new Blob([bcmr.value.getContent()], {
