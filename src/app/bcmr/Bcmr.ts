...
export class Bcmr implements Registry {
    ...
    if (this.registryIdentity && typeof this.registryIdentity === 'string') {
      ...
      if (this.identities) {
        ...
        if (
          ...
          this.identities[registryIdentity][this.latestRevision]
        ) {
  ...
  get nfts(): [{ [commitmentHex: string]: NftType }] | [] {
    if (this.nftCollection?.types) {
      return Object.entries(this.nftCollection?.types).map((nftType) => ({
        [nftType[0]]: nftType[1],
      })) as [{ [commitmentHex: string]: NftType }]; // convert array of arrays to array of objects
    }
    return [];
  }
  ...
  setRegistryName(name: string) {
    if (typeof this.registryIdentity === 'string' && this.identities) {
      this.identities![this.registryIdentity!][this.latestRevision!].name =
        name;
    }
  }
  ...
  setTokenDecimals(decimals: number) {
    if (typeof this.registryIdentity === 'string' && this.identities) {
      this.identities![this.registryIdentity!][
        this.latestRevision!
      ].token!.decimals = Number(decimals || 0);
    }
  }
  ...
  getIdentityHistory(authbase: string): string[] {
    if (this.identities) {
      return Object.keys(this.identities[authbase] || {}).sort(
  ...
  getRegistryUri(): string | undefined {
    if (typeof this.registryIdentity === 'string' && this.identities) {
      return this.identities![this.registryIdentity!][this.latestRevision!].uris
        ?.registry;
    }
  }
  ...
  getIdentityHistoryTimestamps(authbase?: string): ISODateString[] {
    if (!this.identities || Object.keys(this.identities || {}).length == 0)
      return [];
    if (
      typeof this.registryIdentity != 'string' &&
      !authbase &&
      Object.keys(this.identities || {}).length > 1
    ) {
  ...
  createNewIdentitySnapshot(
    ...
    this.newRevision =
      identityHistoryTimestamp || (new Date().toISOString() as ISODateString);
    if (!this.identities) {
  ...
  addNftTypes(
    ...
    for (const key of Object.keys(nftTypes)) {
      this.identities![authbase || (this.registryIdentity as string)][
        identityHistoryTimestamp
      ].token!.nfts!.parse!.types[key] = nftTypes[key];
    }
    return this;
  }
}
