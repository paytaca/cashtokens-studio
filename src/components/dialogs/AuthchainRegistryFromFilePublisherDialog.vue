  ...
  <q-dialog v-close-popup>
    <q-card class="q-px-sm q-py-lg full-width">
      ...
      <q-toolbar class="flex-reverse">
        <q-toolbar-title
          class="col-12 text-h5 text-bold q-mb-md"
          style="text-wrap: wrap"
          >Upload and Publish Registry
        </q-toolbar-title>
      </q-toolbar>
      ...
      <div class="rounded bordered">
        <q-avatar
          class="q-ma-sm"
          v-if="authchainIdentity.identitySnapshot?.uris?.icon"
        >
          <img :src="authchainIdentity.identitySnapshot?.uris?.icon" alt="" />
        </q-avatar>
        <span
          v-if="authchainIdentity.identitySnapshot?.token?.symbol"
          class="q-ma-sm text-bold"
          >{{ authchainIdentity.identitySnapshot?.token?.symbol }}
        </span>
      ...
      <div class="q-ma-md text-justify">
        ...
        <span>
          Upload your token's registry (BCMR) file to IPFS and publish it's URI
          and content hash on-chain.
        </span>
      </div>
      <q-card-section>
        <q-uploader
          @uploaded="onFileUploaded"
          field-name="registryFile"
      ...
      <q-card-actions class="row justify-end">
        <div v-if="uploading" class="row items-end text-warning">
          <i class="q-mr-xs"
            >Storing file in IPFS. This may take a while, please wait</i
          ><q-spinner-dots></q-spinner-dots>
        </div>
        <BusyButton
          v-if="uploaded"
          :busy-label="authchainIdentity?.processing"
          label="Publish"
          color="primary"
          @click="publish"
          :disable="!uploaded || Boolean(authchainIdentity?.processing)"
        />
      </q-card-actions>
    </q-card>
  </q-dialog>
...
const publish = async () => {
  ...
  if (!uploadArtifact?.value?.contentHash) {
    return $q.notify({
      type: 'negative',
      message: 'Missing registry content hash',
    });
  }
  try {
    const tx = await props.authchainIdentity.publish({
      url: uploadArtifact.value!.uris.https!,
    ...
    if (tx) {
      ...
      emit('registryPublished', {
        tokenId: props.authchainIdentity!.token!.tokenId,
      });
      ui.setStatusMessage({
