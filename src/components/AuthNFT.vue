<template>
  <q-card class="full-width">
    ...
    <q-card-section>
      ...
      <q-input v-if="authNft" :model-value="authNft.utxo?.txid" :filled="true" disable dense square />
      <p v-else>No utxo suitable as auth NFT in your address. Please send BCH to your address.</p>
    </q-form>
    </q-card-section>
    <q-card-actions class="justify-end">
      <BusyButton v-if="authNft" @click="createAuthNFTGenesis" :busy-label="authNft?.processing" label="Create Auth NFT"/>
    </q-card-actions>
  </q-card>
