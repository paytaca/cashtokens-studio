...
/**
 * Create a mint transaction with registry publication.
 * 
...
export default async (args: {
    ...
    minterCommitment?:string,
  }): Promise<{decoded: TransactionCommon, sourceOutputs: any}> => {

  if (!args.authkey?.token?.tokenId) {
    throw new Error('Invalid authkey')
  }

  const minerFee = calcMinerFee({'P2SH-P2WPKH':1, P2PKH:2}, {P2SH:1, P2PKH: 4 + args.tokensToMint.length})
  ...
  try {
    ...
    if (args.publish) {
      const opReturnValue = [
        'BCMR',
        args.publish.contentHash,
        ...args.publish.uris.map(u => {
          u.replace('https://', '')
          u.replace('ipfs://', '')
          return u
        })
      ]
      transaction = transaction.withOpReturn(opReturnValue)
    }
