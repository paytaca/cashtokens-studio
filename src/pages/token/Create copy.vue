  ...
  <q-page class="q-pa-lg" style="min-height: 100vh">
    <div class="row justify-center">
      <div class="col-xs-12 col-md-10 col-lg-8">
        ...
        <q-form v-if="options.currentView === 'create-token'" class="row q-gutter-md q-pa-sm">
          <q-input :filled="true" class="col-xs-12" dark:color="lime" v-model="creator" label="Creator"
            aria-disabled="true" disable dense square />
          <div class="col-xs-12">
            <q-select class="overflow-hidden ellipsis" :loading="user.updatingBalances" :filled="true" bottom-slots
              v-model="tokenIdSelected" :options="tokenIdOptions" label="Token ID"
            ...
            <i v-if="!tokenIdSelected?.value" class="text-caption">{{ tokenIdInputHint }}</i>
            <q-btn v-if="tokenIdSelected?.value" size="xs" class="q-mx-sm text-caption"
              @click.stop="copyText(tokenIdSelected.value)" flat dense>Copy Token id</q-btn>
          </div>
          <div v-if="token.tokenId && token.tokenType === 'fungible' || token.tokenType === 'hybrid'" class="col-xs-12">
            <q-input :filled="true" dark:color="lime" v-model="token.amount" min="1" :max="constants.MAX_FUNGIBLE_AMOUNT"
              label="Genesis Supply" aria-disabled="true" dense square>
            ...
            <!-- <div class="col-xs-12">
              <q-checkbox :filled="true" dark:color="lime" v-model="options.useMintingBaton" size="xs"
                label="Store FT reserves in Minting Covenant">
              </q-checkbox>
              <q-btn href="https://github.com/mr-zwets/MBC-Token-Standard" :target="'_blank'" icon="help" size="xs"
                color="info" flat round>
                <q-tooltip>Genesis supply will be locked with a minting covenant. Click to view
                  CHIP-2023-07-MBC-standard</q-tooltip>
              </q-btn>
            </div> -->
            <q-select class="col-xs-12 ellipsis" :loading="user.updatingBalances" :filled="true" bottom-slots
              v-model="options.ftGenesisSupplyStorage"
              :options="['Authchain', 'Minting Baton Covenant', 'Creator Address']" label="Store FT Genesis Supply In"
              dense square hide-bottom-space>
              <template v-slot:option="scope">
                <q-item v-if="scope.opt === 'Authchain'" v-bind="scope.itemProps">
                  {{ scope.opt + ' (BCMR recommended)' }}
                  <q-tooltip>
                    Recommended for continued issuance. Genesis supply will be stored as FT reserve in the
                    authchain
                  </q-tooltip>
                </q-item>
                <q-item v-if="scope.opt === 'Minting Baton Covenant'" v-bind="scope.itemProps">
                  {{ scope.opt }}
                  <q-tooltip>Alternative for continued issuance. Genesis supply will be stored as FT reserve in the
                    Minting Baton Covenant</q-tooltip>
                </q-item>
                <q-item v-if="scope.opt === 'Creator Address'" v-bind="scope.itemProps">
                  {{ scope.opt }}
                  <q-tooltip>Supply will be issued to the creator</q-tooltip>
                </q-item>
              </template>
            </q-select>
          </div>
          <q-select v-if="token.tokenType === 'nonfungible' || token.tokenType === 'hybrid'" class="col-xs-12 q-mb-sm"
...
import { onMounted, ref, computed, watch, LabelHTMLAttributes } from 'vue'
import { utf8ToBin, decodeTransaction, binToHex } from '@bitauth/libauth'
import { hexToBin, BCMR, OpReturnData, SendRequest, TokenSendRequest, UtxoI, TokenI, Wallet, Network } from 'mainnet-js'
...
const tokenIdOptions = computed<{ value: string, label: string }[]>((): { value: string, label: string }[] => {
  return user.genesisInputs?.map((u: UtxoI) => ({
    label: u.txid.replace(u.txid.substring(5, 48), '...'),
    value: u.txid
  })) || []
})

const tokenIdInputHint = computed(() =>
  tokenIdOptions.value && tokenIdOptions.value.length > 0 ?
    'Select token id from suitable UTXOs' : 'No suitable UTXO, please consolidate your UTXOs and try again.'
)
...
const options = ref<{
  ...
  ftGenesisSupplyStorage: 'Authchain',
  data: {
    publishIdentityOutput: {
      registryUrl: '',
      contentHash: ''
...
onMounted(async () => {
  ...
  if (tokenIdOptions.value) {
    tokenIdSelected.value = { value: tokenIdOptions.value[0].value, label: tokenIdOptions.value[0].label }
  }
  if (tokenIdOptions.value) {
    token.value.tokenId = tokenIdOptions?.value[0].value
  }
})

const tokenIdSelectedChanged = (opt: { value: string, label: string }) => {
  token.value.tokenId = opt.value
}
 ...
 */
const prepareIdentityOutputRequest = (recipient: string): (SendRequest | TokenSendRequest | OpReturnData) => {
  let r = new TokenSendRequest({ cashaddr: recipient, value: 1000, tokenId: token.value.tokenId, commitment: binToHex(utf8ToBin('identity')) })
  if (token.value.tokenType === 'fungible' && options.value.ftGenesisSupplyStorage === 'Authchain') {
    // Store supply in identity output
    r = new TokenSendRequest({ cashaddr: recipient, value: 1000, tokenId: token.value.tokenId, amount: Number(token.value.amount), commitment: binToHex(utf8ToBin('identity')) })
  }
  return r
}

/**
 * Prepare the token genesis
 */
const prepareTokenRequest = (creatorWallet: Wallet): (SendRequest | TokenSendRequest | OpReturnData)[] => {
  if (token.value.tokenType === 'fungible' && options.value.ftGenesisSupplyStorage === 'Authchain') {
    // Fungible tokens  in identity output
    return []
  }
  ...
  let tokensRecipient = creatorWallet.getTokenDepositAddress()
  let tokenGenesisRequest: (SendRequest | TokenSendRequest)[] = [new TokenSendRequest({ cashaddr: tokensRecipient, value: 1000, ...genesisTokenFields })]
  if ((token.value.tokenType === 'fungible' || token.value.tokenType === 'hybrid') && options.value.useMintingBaton) {
    const mintingCovenant = new MintingCovenant(token.value.tokenId, creatorWallet.network, $q.notify)
    tokensRecipient = mintingCovenant.contract.getTokenDepositAddress()
    tokenGenesisRequest = [
      // FT Reserves
      new TokenSendRequest({ cashaddr: tokensRecipient, tokenId: token.value.tokenId, value: 1000, amount: Number(token.value.amount) }),
      // Minting Baton NFT
      new TokenSendRequest({ cashaddr: tokensRecipient, tokenId: token.value.tokenId, value: 1000, commitment: '00', amount: 0 })
    ]
  }
...
const prepareBcmrPublicationOutput = (): OpReturnData[] => {
  if (options.value.publishIdentityOutput === true) {
    return [OpReturnData.fromArray(['BCMR', options.value.data.publishIdentityOutput.contentHash, options.value.data.publishIdentityOutput.registryUrl.replace('https://', '')])]
  }
  return []
}
/**
...
const prepareGenesisRequest = async (wallet: Wallet): Promise<(SendRequest | TokenSendRequest | OpReturnData)[]> => {
  ...
  requests.push(prepareIdentityOutputRequest(authchainIdentityOutputRecipient))
  requests.push(...prepareTokenRequest(wallet))
  requests.push(...prepareBcmrPublicationOutput())
