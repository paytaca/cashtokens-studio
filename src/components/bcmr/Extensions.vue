<template>
  <div>
    ...
    <div class="flex">
      <div v-for="key1, i in Object.keys(extensions || {})" :key="'extensions' + i" class="flex items-center">
        <q-chip v-if="typeof (extensions[key1]) == 'string'" removable @remove="removeExtension(extensions, key1)"
          size="lg">
          {{ key1 }}: {{ extensions[key1] }}
        </q-chip>
        <template v-else-if="typeof (extensions[key1]) == 'object'">
          <div v-for="key2, ii in Object.keys(extensions[key1])" :key="'extensions' + ii" class="flex items-center">
            <q-chip removable @remove="removeExtension(extensions, key1, key2)" size="lg">
              {{ key1 }}.{{ key2 }}: {{
        extensions[key1][key2] }}
            </q-chip>
          </div>

        </template>
      </div>
      <q-btn @click="dialog = true" text-color="primary" icon="add" size="md" round></q-btn>
    </div>
    ...
    <q-dialog v-model="dialog" @before-show="onBeforeDialogShow" @before-hide="onBeforeDialogHide" class="q-pa-lg"
      ...
      <q-card>
        ...
        <q-card-section class="q-pt-none q-gutter-lg">
          <q-banner style="background-color: #55454512; color:orange" class="rounded-borders">
            {{ extensionOnDialog }}
          </q-banner>
          <q-select v-model="extensionValueType" :options="['Text', 'Key Value Pair']" label="Value Format" stack-label
            class="text-capitalize" outlined />
          <q-input v-model="extensionOnDialogLabel" label="Label"
            :placeholder="extensionValueType == 'Text' ? 'Example: info' : 'Example: attributes'" outlined autofocus />
          <fieldset v-if="extensionValueType == 'Key Value Pair'" class="q-pa-lg">
            <legend>Value</legend>
            <div class="q-gutter-lg">
              <q-input v-model="extensionOnDialogChildLabel" label="Label" placeholder="Example: color" outlined />
              <q-input v-model="extensionOnDialogValue" label="Value" placeholder="Example: blue" outlined />
            </div>
          </fieldset>
          <div v-else>
            <q-input v-model="extensionOnDialogValue" label="Value" placeholder="Examp  le: additional info" outlined />
          </div>
        </q-card-section>
...
const addExtension = () => {
  ...
  if (key.length > 0) {
    ...
    if (typeof (extensionOnDialog.value[key]) == 'object') {
      console.log('object', extensionOnDialog.value)
      extensions.value[key] = { ...extensions.value[key] as object, ...extensionOnDialog.value[key] }
...
const removeExtension = (extensions: Extensions, key: string, key2?: string) => {
  if (!key2) {
...
watch(() => extensionValueType.value, (v) => {
  extensionOnDialogLabel.value = ''
  if (v == 'Key Value Pair') {
    extensionOnDialogChildLabel.value = ''
  } else {
