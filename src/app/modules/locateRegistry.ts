 ...
 */
export default async ( registryIdentity:OffChainRegistryIdentity | string ): Promise<Registry|undefined> => {
  ...
  const pubInfo = await (new ChainGraph()).retrieveLastRegistryPublication(registryIdentity)
  if (pubInfo && pubInfo[0]) {
    if (pubInfo[0].httpsUrl) {
      try {
        ...
        throw new Error(`Found registry publication but unable to load from the published URL (${pubInfo[0].httpsUrl}). Verify that the URL exist or try again later`)
      }
    }
  }
}