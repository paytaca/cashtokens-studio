  ...
  <q-dialog v-close-popup>
    <q-card class="q-px-sm q-py-lg full-width">
      ...
      <div class="q-mx-md text-justify q-mt-md q-mb-sm">
        ...
        <span>
          This operation will create a minting contract, that'll allow anybody
          with BCH to mint your NFT. Fill up the form below to setup your
          contract.
        </span>
      </div>
      <q-card-section class="q-gutter-sm">
        <q-form id="f-nmcdd" ref="mintFormRef" class="q-gutter-sm">
          <q-input
            ...
            label="Mint price"
            placeholder="How much it cost to mint your NFT?"
            filled
          >
            <template v-slot:prepend>
              <q-avatar size="sm">
                <q-img
                  src="https://chipnet.imaginary.cash/img/logo/bch.svg"
                ></q-img>
              </q-avatar>
            </template>
          </q-input>
          <q-input
            v-model="form.collectionSize"
            label="Collection size"
            placeholder="The total number of NFTs mintable in this collection"
            filled
          ></q-input>
          <q-input
            ref="mintDateRef"
            v-model="form.mintDate"
            label="Go live on (Date)"
            placeholder="The date when the mint goes live on CashTokens Studio"
            type="date"
            :rules="[mintDateRules]"
            bottom-slots
            filled
          ...
          <q-input
            ref="mintTimeRef"
            v-if="form.mintDate && !mintDateRef.hasError"
            v-model="form.mintTime"
            label="At (Time)"
            :rules="[mintTimeRules]"
            placeholder="The date when the mint goes live on CashTokens Studio"
            type="time"
          ...
          ></q-input>
          <q-input
            ref="mintBannerMessageRef"
            v-if="form.mintDate && !mintDateRef.hasError"
            v-model="form.mintBannerMessage"
            label="Banner Message (optional)"
            :rules="[(v) => v.length <= 240 || '240 Characters Max']"
            placeholder="An optional custom message that'll be displayed on your token's minting dialog."
            filled
            bottom-slots
          >
          </q-input>
      ...
      <q-card-actions class="row justify-end">
        <BusyButton
          @click="deployMintingContract"
          label="Deploy Contract"
          :busyLabel="multiThreadedMinter?.processing"
          color="primary"
          :disable="
            disableContractDeployment || Boolean(ctsBackend?.processing)
          "
        />
      </q-card-actions>
    </q-card>
  ...
  try {
    ...
    if (tx) {
      ...
      const pubRes = await ctsBackend.value.publishNFTProject({
        ...
        mintingContractParams:
          multiThreadedMinter.value.contractScriptParams.map((p: any) =>
...
const disableInputs = (disable: boolean) => {
  if (disable) {
    document
      .querySelectorAll('#f-nmcdd input')
      .forEach((e: any) => (e.disabled = true));
  } else {
    document
      .querySelectorAll('#f-nmcdd input')
      .forEach((e: any) => (e.disabled = ''));
  }
};

watch(
  () => multiThreadedMinter.value?.processing,
  (v) => {
