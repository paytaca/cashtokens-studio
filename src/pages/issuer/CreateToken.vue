      ...
      <div class="col-xs-12 col-sm-10 col-lg-9">
        <div class="row justify-center q-my-lg">
          <template v-if="!genesisInputUtxo || !authKey">
            <q-banner
              class="q-mt-lg q-px-lg q-py-xl text-justify"
            ...
            >
              ...
              <p>
                Creating a new token requires a "genesis input". A valid genesis
                input is just a utxo that is the first output(v-out 0) of a
                previous transaction.
              </p>
              <p>
                Currently
                <span class="text-negative">
                  your wallet have {{ user.genesisInputs?.length }}</span
                >
                utxo that we can use as genesis input. This operation
                <span class="text-positive">requires 2</span> genesis input (1
                for your token, 1 for an AuthKey token). You can create a
                genesis input by clicking the button below. If you want to use
                an existing AuthKey
                <q-btn
                  to="/issuer/manage/authkeys"
                  flat
                  color="secondary"
                  dense
                  no-caps
              ...
              <template v-slot:action>
                <BusyButton
                  :busy-label="genesisInputInstance?.processing"
                  label="Generate genesis input"
          ...
          <template v-else>
            <TokenGenesisForm
              ...
              :genesis-input="genesisInputUtxo"
              :auth-key="authKey"
              :owner-wallet="user.wallet! as Wallet"
              :create-auth-key="true"
              @genesis-result="onGenesisResult"
            />
          </template>
        </div>
      </div>
...
const tokenType = computed(
  () => route.params.tokenType as 'ft' | 'nft' | 'fnft',
);
...
const generateGenesisInputs = async () => {
  ...
  const transactionSigner: TransactionSigner =
    user.walletType === 'paytaca'
      ? paytacaConnect.paytacaTransactionSigner
      : walletConnect.walletConnectTransactionSigner;

  try {
    genesisInputInstance.value = new GenesisInput(
      { vout: 0, satoshis: 0, txid: '' },
      transactionSigner,
    ); //
    const tx = await genesisInputInstance.value.generate(
...
const onGenesisResult = async (_result: any) => {};
